...
- **go_bundle** - A Go HTTP filter loaded at runtime by the Go Bundle dynamic module (default).
- **dynamic_module_rust** - A Rust HTTP filter compiled as a native dynamic module.

### Creating a Go (Bundle) Extension

```bash
boe create my-extension --path ~/src/extensions
```

This is equivalent to explicitly passing `--type go_bundle`. The generated project structure looks like this:
...
```bash
boe create my-extension --type dynamic_module_rust --path ~/src/extensions
```
