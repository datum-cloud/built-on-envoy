...
Version of Built On Envoy Go Bundle. Required if type is `go_bundle` unless the `parent` field is set.
...
| **Required when** | `type` is `go_bundle` and `parent` is not set |
...
Parent extension set whose version will be used. When set, the `version` and `goBundleVersion` fields can't be set.
...
| **Type** | `string` |
| **Required** | Yes |
| **Allowed values** | `lua`, `wasm`, `dynamic_module`, `go_bundle` |
