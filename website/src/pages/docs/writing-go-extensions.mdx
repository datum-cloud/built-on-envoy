...
**Important:** The `goBundleVersion` field must match the version of the Go Bundle dynamic module that will load your plugin.
...
Go plugins have strict version requirements. See the [Extension types](/docs/extension-types) page for further details
about version compatibility and how the internals work.

The `goBundleVersion` in your manifest indicates which Go Bundle version your plugin is compatible with.
When the Go Bundle loads your plugin, it validates these requirements.

## Complete Example

For a complete example, refer to the [example-go](https://github.com/tetratelabs/built-on-envoy/tree/main/extensions/go-bundle/example/) extension in GitHub.
