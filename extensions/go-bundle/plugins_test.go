...
type Manifest struct {
	...
	GoBundleVersion string `yaml:"goBundleVersion,omitempty"`
...
func validateManifest(t *testing.T, path string, knownExtensions map[string]bool) {
	...
	// Validate type
	require.Equal(t, "go_bundle", manifest.Type, "type must be 'go_bundle'")

	// Validate version.
	require.Empty(t, manifest.Version, "in-tree go_bundle plugins should not have a version")

	require.Empty(t, manifest.GoBundleVersion, "in-tree go_bundle plugins should not have a goBundleVersion")

	// Validate parent.
	require.Equal(t, "gobundle", manifest.Parent, "go_bundle plugins must have parent gobundle")
}
