...
This directory contains the embedded version of the example Go plugin, compiled directly into the Go Bundle dynamic module binary.

See the [example-go README](../README.md) for details on the plugin implementation and packaging approaches.

## How it works

The `host.go` file imports the plugin package and registers it with the Envoy SDK during initialization. The Go Bundle's `main.go` imports this package to include the plugin in the final binary.
...
> which is used to build `libgobundle.so` as well.
