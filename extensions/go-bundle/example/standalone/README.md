...
This directory contains the standalone version of the example Go plugin, compiled as a separate `.so` file that can be loaded by the Go Bundle at runtime.
...
This produces `example-go.so` which can be loaded by the Go Bundle's goplugin loader.

> [!Caution]
> The plugin must be compiled with the exact same Go version and dependency versions as the Go Bundle dynamic module.
