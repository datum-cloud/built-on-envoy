...
This directory contains an example Go plugin for Envoy HTTP filters using the Go Bundle Dynamic Module system. It demonstrates two packaging approaches: embedding the plugin directly into the Go Bundle binary and compiling it as a standalone Go plugin.

## Directory Structure

```
example/
├── embedded/            # Embedded packaging (compiled into Go Bundle)
│   ├── host.go
...
- The Go Bundle's [goplugin loader](../goplugin/goplugin.go) loads the `.so` file at runtime
- Before loading, the loader validates that the plugin was built with the same Go version and matching dependency versions

**Advantages:**
- Plugins can be updated independently of the Go Bundle module
- Supports dynamic plugin discovery and loading

**Disadvantages:**
- **Critical constraint:** The plugin must be compiled with the exact same Go runtime version as the Go Bundle
...
### 2. Embedded in Go Bundle (`embedded/`)

The plugin is compiled directly into the Go Bundle dynamic module binary.

**How it works:**
- [embedded/host.go](embedded/host.go) imports the plugin package and registers it with the SDK during initialization
- The Go Bundle's [plugins.go](../plugins.go) imports the embedded package, including it in the final binary
...
**Disadvantages:**
- Requires rebuilding the entire Go Bundle module to update the plugin
